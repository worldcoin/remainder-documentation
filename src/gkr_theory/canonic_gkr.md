...
$$
\widetilde{V}_i(g) = \sum_{z \in \{0, 1\}^{s_i}}\sum_{x \in \{0, 1\}^{s_j}} \widetilde{\text{eq}}(g, z) \cdot \text{id}(z, x) \cdot \widetilde{V}_j(x).
$$
As observed in [XZZ+19](https://eprint.iacr.org/2019/317.pdf), we only need to sum over the wirings which are non-zero (i.e., there exists a re-routing from label $x$ in layer $j$ to label $z$ in layer $i$). Call the set of non-zero wirings as $\mathcal{N}$ (here, we say that $\mathcal{N} \in O(2^{s_i} + 2^{s_j})$, i.e. a constant number of wires for each input layer and output layer value). We can rewrite the summation as:
$$
\widetilde{V}_i(g) = \sum_{(z, x) \in \mathcal{N}} \widetilde{\text{eq}}(g, z) \cdot \widetilde{V}_j(x).
$$
...
We can rewrite this as:
$$
\widetilde{V}_i(g) = \sum_{z \in \{0, 1\}^{s_i}}\sum_{x \in \{0, 1\}^{s_j}}\sum_{y \in \{0, 1\}^{s_k}} \widetilde{\text{eq}}(g, z) \cdot \text{mul}(z, x, y) \cdot \bigg[\widetilde{V}_j(x) \cdot \widetilde{V}_k(y)\bigg]
$$
As observed in [XZZ+19](https://eprint.iacr.org/2019/317.pdf), we only need to sum over the wirings which are non-zero (i.e., there exists a an addition from label $x$ in layer $j$ and label $y$ in layer $k$ to label $z$ in layer $i$). Call the set of non-zero wirings as $\mathcal{N}$ (we assume that there are at most $\mathcal{N} \in O(2^{s_i} + 2^{s_j} + 2^{s_k})$ nonzero mul gate values). We can rewrite the summation as:
$$
\widetilde{V}_i(g) = \sum_{(z, x, y) \in \mathcal{N}} \widetilde{\text{eq}}(g, z) \cdot \bigg[\widetilde{V}_j(x) \cdot \widetilde{V}_k(y)\bigg]
$$

The prover cost for sumcheck over a mul gate layer is as follows:

- The prover must first compute the evaluations of $\sum_{y, z} \widetilde{\text{eq}}(g, z) \cdot \widetilde{V}_j(x) \cdot \widetilde{V}_k(y)$ for $x$ where $(z, x, y) \in \mathcal{N}$. [XZZ+19](https://eprint.iacr.org/2019/317.pdf) splits this pre-processing into two phases (note that proving the $\text{add}$ gate layer follows the same strategy). First, we precompute (or stream) the values in $\widetilde{\text{eq}}(g, z)$ in $O(2^{s_i})$. 
- Next, we compute the "phase 1" preprocessing, where we sumcheck over the $x$ variables. Here, we can directly evaluate $\widetilde{V}_j(x)$ while "folding" $\widetilde{\text{eq}}(g, z) \cdot \widetilde{V}_k(y)$ by summing over the $y$ variables. Similarly, in the second phase, when we compute sumcheck messages over $y$ and already have evaluations for $\widetilde{V}_k(y)$, we can "fold" $\widetilde{\text{eq}}(g, z) \cdot \widetilde{V}_j(x)$ by summing over the $x$ variables. Both of these preprocessing steps take $O(\mathcal{N}) = O(2^{s_i} + 2^{s_j} + 2^{s_k})$ time. 
- After preprocessing, the prover must compute sumcheck messages for the above relationship. Similarly to the preprocessing step above, sumcheck is done in two phases. First, the prover binds the $x$ variables, and then it binds the $y$ variables. The degree of each sumcheck message is $d = 2$, and thus the prover sends $d + 1 = 3$ evaluations per round of sumcheck (this is the same for add gate). Since we are sumchecking over $x \in \{0, 1\}^{s_j}$ and $y \in \{0, 1\}^{s_k}$, there are $s_j + s_k$ rounds of sumcheck and thus the prover cost is $d(d + 1) 2^k$ for the $k$'th round of sumcheck. The total prover sumcheck cost is thus
$$
