...
[Recall](/gkr_theory/matmult_layer.md) that a `MatMult` layer requires all dimensions of all the
matrices involved in the product to be **exact powers of two**.  We can always guarantee this
property by padding the original matrices with zero columns and/or rows as follows:
...
This is easy to do with an [Identity Gate layer](/gkr_theory/canonic_gkr.md#identity-gate),  as
...
![The wirings of the Identity Gate Layer](/figures/matmult_rewiring.png)

Here's the complete code for this example, which can also be found in our repository
at[`frontend/examples/matmult.rs`](https://github.com/worldcoin/Remainder_CE/blob/v1.0.0/frontend/examples/matmult.rs):
...
**Note**: In the previous example we hard-coded the wirings corresponding to the case of padding a
*$3 \times 3$ matrix MLE. For the general case, one can easily generate the correct wirings for
*padding for any matrix dimensions.