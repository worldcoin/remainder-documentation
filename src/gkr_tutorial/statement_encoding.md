...
These binary operations are usually referred to as "gates." In the following tutorial we will be focusing on two gates: $\text{add}$ gates, which are represented by the following function:

$$
    \text{add}(z, x, y): \{0, 1\}^{s_i + s_j + s_k} \mapsto \{0, 1\} = 
\begin{cases}
...
and $\text{mul}$ gates:
$$
    \text{mul}(z, x, y): \{0, 1\}^{s_i + s_j + s_k} \mapsto \{0, 1\} = 
\begin{cases}
...
In this case, $\text{add}(4, 0, 1) = 1$ and $\text{mul}(4, 0, 1) = 0$, but $\text{mul}(7, 4, 1) = 1,$ for example. Notice how the circuit naturally falls in "layers" based on the dependencies of values.  